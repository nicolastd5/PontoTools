import { useState, useEffect } from 'react';
import { useGpsContext } from '../contexts/GpsContext';

interface Unit {
  latitude: number;
  longitude: number;
  radiusMeters: number;
}

function haversineDistance(lat1: number, lon1: number, lat2: number, lon2: number): number {
  const R = 6371000;
  const dLat = ((lat2 - lat1) * Math.PI) / 180;
  const dLon = ((lon2 - lon1) * Math.PI) / 180;
  const a =
    Math.sin(dLat / 2) ** 2 +
    Math.cos((lat1 * Math.PI) / 180) *
    Math.cos((lat2 * Math.PI) / 180) *
    Math.sin(dLon / 2) ** 2;
  return R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
}

export function useGeolocation(unit: Unit | null | undefined) {
  const { status, coords } = useGpsContext();
  const [distanceMeters, setDistanceMeters] = useState<number | null>(null);

  useEffect(() => {
    if (coords && unit?.latitude != null && unit?.longitude != null) {
      const dist = haversineDistance(coords.latitude, coords.longitude, unit.latitude, unit.longitude);
      setDistanceMeters(Math.round(dist * 10) / 10);
    } else {
      setDistanceMeters(null);
    }
  }, [coords, unit]);

  const isInsideZone = Boolean(
    unit?.radiusMeters && distanceMeters !== null && distanceMeters <= unit.radiusMeters,
  );

  return { status, coords, distanceMeters, isInsideZone };
}
