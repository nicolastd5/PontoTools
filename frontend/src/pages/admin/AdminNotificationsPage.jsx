    ...
    <div>
      <div style={s.header}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {unread > 0 && (
            <span style={s.unreadBadge}>{unread} não lidas</span>
          )}
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button onClick={() => setSendModal(true)} style={s.primaryBtn}>+ Enviar Notificação</button>
        </div>
      </div>
              ...
              {notifications.map((n) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ ...s.td, fontWeight: n.read ? 400 : 700, color: n.read ? '#64748b' : '#0f172a' }}>
                      {!n.read && <span style={s.unreadDot} />}
                      {n.title}
                    </td>
                    <td style={{ ...s.td, maxWidth: 240, fontSize: 13, color: '#64748b', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{n.body}</td>
                    <td style={s.td}>
                      {n.push_sent
        ...
        <div style={overlay} onClick={() => setSelected(null)}>
          <div style={modalCard} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
              <div>
                <span style={{ ...badge, ...(TYPE_COLOR[selected.type] || TYPE_COLOR.manual), marginBottom: 10, display: 'inline-block' }}>
                  {TYPE_LABEL[selected.type] || selected.type}
                </span>
                <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', marginTop: 8, lineHeight: 1.4 }}>
                  {selected.title}
                </h2>
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '14px 16px', background: '#f8fafc', borderRadius: 8 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 20 }}>
              <button onClick={() => setSelected(null)} style={s.primaryBtn}>Fechar</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={overlay} onClick={() => setSendModal(false)}>
          <div style={modalCard} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              ...
              <button onClick={() => setSendModal(false)} style={s.closeBtn}>✕</button>
            </div>
            <form onSubmit={handleSend} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 4 }}>
                <button type="button" onClick={() => setSendModal(false)} style={s.outlineBtn}>Cancelar</button>
...
const modalCard = { background: '#fff', borderRadius: 12, padding: '24px', width: '100%', maxWidth: 480, boxShadow: '0 20px 60px rgba(0,0,0,0.2)', maxHeight: '90vh', overflowY: 'auto' };
...
const s = {
  ...
  unreadBadge: { background: '#ef4444', color: '#fff', borderRadius: 20, padding: '2px 9px', fontSize: 12, fontWeight: 700 },
  unreadDot:   { display: 'inline-block', width: 7, height: 7, borderRadius: '50%', background: '#1d4ed8', marginRight: 6, verticalAlign: 'middle' },
  ...
  closeBtn:    { background: 'none', border: 'none', fontSize: 18, cursor: 'pointer', color: '#94a3b8', lineHeight: 1, padding: '2px 4px' },
};
